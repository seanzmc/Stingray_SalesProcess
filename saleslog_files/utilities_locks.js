...
function acquireScriptLockWithRetry(
  ...
  if (initialDelayMs <= 0) {
    Logger.log(
      '[Lock] Invalid initialDelayMs (' + initialDelayMs + '), using 100'
    );
    initialDelayMs = 100;
  }
  if (backoffMultiplier < 1) {
    Logger.log(
      '[Lock] Invalid backoffMultiplier (' + backoffMultiplier + '), using 2'
    );
    backoffMultiplier = 2;
  }
  if (timeoutMs <= 0) {
  ...
  for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
    ...
    try {
      ...
      Logger.log(
        '[Lock] Error on attempt ' + attempt + ': ' + error.toString()
      );
      attemptDetails.push({
        attemptNumber: attempt,
