...
/**
 * UPDATED: Opens the Rewind Reason dialog instead of acting immediately
 */
function menuRewindPointer() {
  try {
    const htmlTemplate = HtmlService.createTemplateFromFile('RewindDialog');
    htmlTemplate.reasons = REWIND_REASONS;
    const html = htmlTemplate.evaluate()
      .setWidth(400)
      .setHeight(250);
    SpreadsheetApp.getUi().showModalDialog(html, 'Rewind Pointer');
  } catch (err) {
...
/**
 * NEW: Processing function called by RewindDialog.html
 * Rewinds pointer, logs to Audit, AND logs to the last Appointment row.
 */
function processMenuRewind(reason) {
  const lockResult = acquireScriptLockWithRetry();
  if (!lockResult.success) {
    SpreadsheetApp.getUi().alert('System busy. Please try again.');
    logError('processMenuRewind', 'Lock acquisition failed');
    return;
  }
  ...
  try {
    ...
    logRoundRobinEvent(AUDIT_ACTIONS.UNDO, {
      ...
      target: 'Pointer',
      reason: reason || 'User requested rewind'
    });

    // 3. Appointment Log (Edge Case Fix)
    // Find the last row in APPOINTMENTS and append note
    try {
      const apptsSheet = getApptsSheet_();
      const lastRow = apptsSheet.getLastRow();
      if (lastRow >= 2) {
        const noteRange = apptsSheet.getRange(lastRow, COL_NOTES);
        const oldNote = noteRange.getValue();
        const suffix = ` [Rewind: ${reason}]`;

        // Append only if not already there to avoid dupes if clicked twice quickly
        if (!String(oldNote).includes(suffix)) {
          noteRange.setValue(String(oldNote) + suffix);
        }
      }
    } catch (noteErr) {
       logError('processMenuRewind_noteUpdate', noteErr);
       // Do not fail the whole operation if note update fails
    }
    ...
    logError('processMenuRewind', err);
